...
async def render_todo_page(request: Request, db: dp_dependecy):
    try:
...
async def render_add_todo_page(request: Request, db: dp_dependecy):
    try:
        user = await get_current_user(request.cookies.get("access_token"))
        if user is None:
            return redirect_to_login()
...
async def render_edit_todo_page(request: Request, todo_id: int, db: dp_dependecy):
    try:
        user = await get_current_user(request.cookies.get("access_token"))
        if user is None:
            return redirect_to_login()
        ...
        return redirect_to_login()
@router.get("/")
async def read_all(user: user_dependecy,db: dp_dependecy):
      if user is None:
          raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
      return db.query(Todo).filter(Todo.owner_id == user.get('id')).all()

@router.get("/get_by_id/{todo_id}", status_code=status.HTTP_200_OK)
async def read_by_id(user: user_dependecy,db: dp_dependecy, todo_id: int=Path(gt=0)):
    if user is None:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
...
async def create_todo(user: user_dependecy,db: dp_dependecy, todo_request: TodoRequest):
    if user is None:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
...
async  def delete_todo(user: user_dependecy,db: dp_dependecy, todo_id: int = Path(gt=0)):
    if user is None:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
    todo = db.query(Todo).filter(Todo.id == todo_id).filter(Todo.owner_id == user.get('id')).first()
    if todo is None:
